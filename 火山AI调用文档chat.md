...
取值范围：`high`、`low`。
doubao\-seed\-1.8 之前的模型默认值为`low`，doubao\-seed\-1.8 模型默认值为`high`。
理解图片的精细度及不同模型对应的具体像素区间，参见[理解图片的深度控制](https://www.volcengine.com/docs/82379/1362931#bf4d9224)。

* `high`：高细节模式，理解图片精细信息，如图片的多个局部信息/特征提取、复杂/丰富细节的图片理解等，更细节，耗时更长。
* `low`：理解图片概略信息，如图片分类/识别、整体内容理解/描述，延时低、花费少。
...
模型消息中思维链内容。
> 仅模型 `doubao-seed-1-8-251228`、`deepseek-v3-2-251201`支持该字段。
...
> 支持字段的模型、使用说明与 **thinking.type** 字段关系见[文档](https://www.volcengine.com/docs/82379/1449737#.6LCD6IqC5oCd6ICD6ZW_5bqm)。
